...
Currency is INR. Use ₹ symbol and format large numbers as Cr / L / K.
"""
...
def _retrieve_context(question: str) -> str:
    ...
    try:
        ...
        if k:
            parts.append(
                ...
                f"  Total outstanding: ₹{k['total_outstanding']:,.0f}\n"
                f"  Open invoices: {k['open_invoices']}\n"
                f"  Overdue amount: ₹{k['overdue_amount']:,.0f} across "
    ...
    for cname in hit_clients:
        ...
        parts.append(
            ...
            f"₹{open_inv['sum_']:,.0f} outstanding\n"
        ...
        try:
            ...
            if m and not scored.empty:
                ...
                parts.append(
                    ...
                    f"totalling ₹{hi['balance'].sum():,.0f}\n"
                    f"  Portfolio expected loss: ₹{el:,.0f}\n"
                ...
                for _, r in top.iterrows():
                    parts.append(
                        ...
                        f"₹{r['balance']:,.0f} · P(late) {r['p_late']:.0%}"
                    )
        except Exception:
            pass
...
@st.dialog("💬 CFO Copilot Assistant", width="large")
def _chat_dialog():
    ...
    st.markdown(
        ...
        "<div>"
        "<div class='cfo-bot-header-name'>CFO Copilot</div>"
